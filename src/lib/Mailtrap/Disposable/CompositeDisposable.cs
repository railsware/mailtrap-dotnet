...
public sealed class CompositeDisposable : ICollection<IDisposable>, ICancelable
{
    ...
    public void Clear()
    {
        ...
        lock (_gate)
        {
            ...
            if (current is List<IDisposable?> currentList)
            {
                previousDisposables = [.. currentList];
                currentList.Clear();
            }
            else
            {
    ...
    public IEnumerator<IDisposable> GetEnumerator()
    {
        lock (_gate)
        {
            ...
            return new CompositeEnumerator(current is List<IDisposable?> currentList ? [.. currentList] : ((Dictionary<IDisposable, int>)current).Keys.ToArray());
        }
    }
